...
<body class="bg-body-secondary">
    ...
    <div class="cadastro-ambiente bg-light rounded-5 position-absolute top-50 start-50 translate-middle">
        ...
        <form action="<?= site_url('gerador/editar/salvar/') . $gerador['ID']?>" method="post" enctype="multipart/form-data">
            <div class="d-flex flex-wrap justify-content-between">
                <?= csrf_field() ?>
                <div class="formulario col col-12 col-md-6 my-3">
                    <h1>Editar Gerador</h1>
                    <div class="my-4">
                        <div class="linha">
                            <input type="text" name="nome" placeholder="Nome do Gerador" value="<?= esc($gerador['DESCRICAO'])?>" required>
                            <input type="number" name="energiaGerada" placeholder="Energia Gerada (KWh)" value="<?= esc($gerador['ENERGIA_GERADA'])?>" required>
                        </div>
                        <div class="linha">
                            <select name="redeEletrica">
                                <option value="<?= esc($redeEletricaGerador['ID']) ?>" selected><?= esc($redeEletricaGerador['DESCRICAO']) ?></option>
                                <?php foreach ($redesEletricas as $redeEletrica): ?>
                                    <option value="<?= esc($redeEletrica['ID']) ?>"><?= esc($redeEletrica['DESCRICAO']) ?></option>
                                <?php endforeach; ?>
                            </select>
                            <select name="tipo">
                                <option value="<?= esc($gerador['TIPO'])?>"><?= esc(($gerador['TIPO']) == 'R') ? 'Renovável' : 'Não Renovável'?></option>
