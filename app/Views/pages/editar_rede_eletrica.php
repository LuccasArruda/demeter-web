...
<body class="bg-body-secondary">
    ...
    <div class="cadastro-ambiente bg-light rounded-5 position-absolute top-50 start-50 translate-middle">
        ...
        <form action="<?= site_url('rede-eletrica/editar/salvar/' . $redeEletrica['ID'])?>" method="post" enctype="multipart/form-data">
            <div class="d-flex flex-wrap justify-content-between">
                <?= csrf_field() ?>
                <div class="formulario col col-12 col-md-6 my-3 d-flex align-items-center">
                    <div>
                        <h1>Editar Rede Elétrica</h1>
                        <div class="linha">
                            <input type="text" name="descricao" value="<?= esc($redeEletrica['DESCRICAO']) ?>" placeholder="Descrição da Rede Elétrica" required>
                            <select name="ambiente">
                                <option value="<?= esc($ambienteRedeEletrica['ID']) ?>"><?= esc($ambienteRedeEletrica['DESCRICAO']) ?></option>
